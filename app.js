...
function showPage(name) {
  if (name === 'home') { loadStats(); loadVivante(); loadHomeProjects(); loadAdminPublicInfo(); loadRevueHomePreview(); }
  if (name === 'contact') {
...
if (backBtn) backBtn.style.display = (name === 'home') ? 'none' : 'flex';
 ...
 if (name === 'home') { loadStats(); loadVivante(); loadAdminPublicInfo(); loadRevueHomePreview(); }
    if (name === 'chambres') {} 
...
function loadShell() {
  ...
  fetch('/partials/shell.html')
    ...
    .then(function(html) {
      slot.innerHTML = html;
      refreshNavUI();
      // Filet de sécurité : sur connexion lente, le profil (rôle admin/
      // enseignant) peut arriver après l'injection du menu, ou l'inverse.
      // On réapplique l'état du menu à deux reprises pour rattraper la course.
      setTimeout(refreshNavUI, 600);
      setTimeout(refreshNavUI, 1600);
    })
...
if (badgeGal) badgeGal.style.display = isVisitor ? 'inline' : 'none';
  ...
  // Intervenants : vitrine publique, visible pour tout le monde quel que soit le rôle
  var menuIntervenantsUser = document.getElementById('menu-intervenants');
  if (menuIntervenantsUser) menuIntervenantsUser.classList.remove('hidden');

  // Forum : réservé aux student/teacher/admin — pas aux visitor
  var menuForumUser = document.getElementById('menu-forum');
  if (menuForumUser) {
    if (isVisitor) menuForumUser.classList.add('hidden');
    else menuForumUser.classList.remove('hidden');
  }
}
...
if (menuSettings) menuSettings.classList.add('hidden');
  ...
  // Intervenants : page vitrine réservée aux non-connectés et aux comptes visiteur
  var menuIntervenantsGuest = document.getElementById('menu-intervenants');
  if (menuIntervenantsGuest) menuIntervenantsGuest.classList.remove('hidden');
  // Forum : réservé aux connectés (student/teacher/admin) — masqué pour les non-connectés
  var menuForumGuest = document.getElementById('menu-forum');
  if (menuForumGuest) menuForumGuest.classList.add('hidden');
}
...
if (e6) e6.textContent = res.data.bio || '';
      ...
      // Page Paramètres — aperçu avatar + champs, toujours synchronisés
      var settingsAvatarImg    = document.getElementById('settings-avatar-img');
      var settingsAvatarLetter = document.getElementById('settings-avatar-letter');
      var settingsRemoveBtn    = document.getElementById('settings-avatar-remove-btn');
      if (res.data.avatar_url) {
        if (settingsAvatarImg)    { settingsAvatarImg.src = res.data.avatar_url; settingsAvatarImg.style.display = 'block'; }
        if (settingsAvatarLetter) settingsAvatarLetter.style.display = 'none';
        if (settingsRemoveBtn)    settingsRemoveBtn.style.display = 'block';
      } else {
        if (settingsAvatarImg)    settingsAvatarImg.style.display = 'none';
        if (settingsAvatarLetter) { settingsAvatarLetter.style.display = 'block'; settingsAvatarLetter.textContent = fn.charAt(0).toUpperCase(); }
        if (settingsRemoveBtn)    settingsRemoveBtn.style.display = 'none';
      }
      var settingsBioInput    = document.getElementById('settings-bio-input');    if (settingsBioInput)    settingsBioInput.value = res.data.bio || '';
      var settingsClassInput  = document.getElementById('settings-class-input');  if (settingsClassInput)  settingsClassInput.value = res.data.class_level || '';
      var settingsSchoolInput = document.getElementById('settings-school-input'); if (settingsSchoolInput) settingsSchoolInput.value = res.data.school || '';
    }
...
async function loadStats() {
  if (!document.getElementById('slideshow')) return; // page d'accueil uniquement
...
if (document.getElementById('slideshow')) { loadStats(); }
...
/* ---------- Bloc admin public — bas de page d'accueil, info en dur ---------- */
async function loadAdminPublicInfo() {
  var block = document.getElementById('admin-public-block');
  if (!block) return;
  try {
    var res = await sb.from('profiles')
      .select('full_name, avatar_url, bio, school, class_level')
      .eq('role', 'admin')
      .limit(1)
      .single();
    var admin = res.data;
    if (!admin) { block.classList.add('no-admin'); return; }
    block.classList.remove('no-admin');

    var avatarEl = document.getElementById('admin-public-avatar');
    if (avatarEl) {
      if (admin.avatar_url) avatarEl.innerHTML = '<img src="' + admin.avatar_url + '" alt="">';
      else avatarEl.textContent = (admin.full_name || 'A').charAt(0).toUpperCase();
    }
    var nameEl = document.getElementById('admin-public-name');
    if (nameEl) nameEl.textContent = admin.full_name || 'Administrateur';

    var metaParts = [];
    if (admin.school) metaParts.push(admin.school);
    if (admin.class_level) metaParts.push(admin.class_level);
    var metaEl = document.getElementById('admin-public-meta');
    if (metaEl) metaEl.textContent = metaParts.join(' \u00b7 ');

    var bioEl = document.getElementById('admin-public-bio');
    if (bioEl) bioEl.textContent = admin.bio || '';
  } catch(e) {
    block.classList.add('no-admin');
    console.warn('loadAdminPublicInfo error:', e);
  }
}

/* ---------- Aperçu Revue — 3 dernières publications sur l'accueil ---------- */
async function loadRevueHomePreview() {
  var wrap = document.getElementById('revue-home-preview');
  if (!wrap) return;
  try {
    var res = await sb.from('revue_posts')
      .select('id, title, summary, cover_image_url, category:revue_categories(label)')
      .eq('status', 'published')
    ...
    for (var i = 0; i < posts.length; i++) {
      ...
      var img = p.cover_image_url
        ? '<img src="' + p.cover_image_url + '" style="width:100%;aspect-ratio:16/9;object-fit:cover;display:block">'
        : '<div style="width:100%;aspect-ratio:16/9;background:var(--gris-light)"></div>';
      html += '<div style="cursor:pointer;border:1px solid rgba(160,120,70,0.15);border-radius:0.5rem;overflow:hidden;background:#fff" onclick="window.location.href=\'/revue/article.html?id=' + p.id + '\'">';
      html += img;
      html += '<div style="padding:0.8rem 1rem">';
      if (p.category) html += '<p style="font-size:0.62rem;letter-spacing:0.06em;text-transform:uppercase;color:var(--ocre);font-family:sans-serif;font-weight:600;margin-bottom:0.3rem">' + p.category.label + '</p>';
      html += '<h4 style="font-family:\'Cormorant Garamond\',serif;font-size:1.05rem;color:var(--terre);line-height:1.3">' + p.title + '</h4>';
      html += '</div></div>';
    }
    wrap.innerHTML = html;
  } catch(e) { console.warn('loadRevueHomePreview error:', e); }
}
...
async function openEditProject(projectId) {
  window.location.href = '/projet/editer.html?id=' + projectId;
}
...
async function openProjectDetail(projectId) {
  window.location.href = '/projet/?id=' + projectId;
}
// ═══════════════════════════════════════════════════════════════════
...
/* ---------- Page Paramètres : bio / classe / école ----------
   Toute modification enregistrée ici se répercute immédiatement
   sur le profil (galerie personnelle, et bloc admin public si role=admin)
   via loadProfileData(), qui relit la table 'profiles'. */
async function doSaveSettingsProfile() {
  if (!currentUser) { return; }
  var bio         = document.getElementById('settings-bio-input').value;
  var classLevel  = document.getElementById('settings-class-input').value;
  var school      = document.getElementById('settings-school-input').value;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  try { if (currentProfile) loadThemeFromProfile(currentProfile); } catch(e) {}
    ...
    // Redirection en provenance des pages autonomes /galerie/ et /projets/
    var deepLinkParams = new URLSearchParams(window.location.search);
    var openProjectId  = deepLinkParams.get('openProject');
    var editProjectId  = deepLinkParams.get('editProject');
    var loginRequested = deepLinkParams.get('login');
    if (openProjectId) {
      history.replaceState(null, '', window.location.pathname);
      openProjectDetail(openProjectId);
    } else if (editProjectId) {
      history.replaceState(null, '', window.location.pathname);
      openEditProject(editProjectId);
    } else if (loginRequested && !currentUser) {
      history.replaceState(null, '', window.location.pathname);
      showPage('login');
    }
  });
...
document.querySelectorAll('.auth-page').forEach(function(el) {
  ...
  loadStats();
loadVivante();
loadAdminPublicInfo(); loadRevueHomePreview();
;
...
var ATELIER_IMGS = [
  ...
  requireAuth(function() {
    ...
    window.location.href = '/ateliers/?type=' + type;
  }, false);
}
...
function openStudentGallery(userId) {
  ...
  window.location.href = '/galerie/?user=' + (userId || currentUser.id);
}
